...
async function createDemotivator(inputPath, caption) {
  try {
    ...
    if (lines.length === 1) {
      ...
      baseFilter += `,drawtext=text='${escapedText}':fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf:fontsize=${fontSize}:fontcolor=white:x=(w-text_w)/2:y=h-${blackBorderBottom/2}-text_h/2`;
    } else {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        baseFilter += `,drawtext=text='${escapedLine}':fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf:fontsize=${fontSize}:fontcolor=white:x=(w-text_w)/2:y=${yPos}`;
      }
    }
